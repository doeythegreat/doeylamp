...
def display_image(face, class_vector=None,
    ...
    if len(face.shape) < 3:
        color_map = 'gray'
    matplot.pyplot.figure()
    if class_vector is not None:
        class_arg = np.argmax(class_vector)
        class_name = class_decoder[class_arg]
        matplot.pyplot.title(class_name)
    if pretty:
        pretty_imshow(matplot.pyplot.gca(), face, cmap=color_map)
    else:
        matplot.pyplot.imshow(face, color_map)
...
def draw_mosaic(data, num_rows, num_cols, class_vectors=None,
    ...
    figure, axis_array = matplot.pyplot.subplots(num_rows, num_cols)
...
if __name__ == '__main__':
    ...
    pretty_imshow(matplot.pyplot.gca(), make_mosaic(faces[:4], 2, 2), cmap='gray')
    matplot.pyplot.show()
    ...
    matplot.pyplot.figure(figsize=(15, 15))
    matplot.pyplot.title('conv1 weights')
    pretty_imshow(matplot.pyplot.gca(),
            make_mosaic(kernel_conv1_weights, box_size, box_size),
            cmap=matplot.cm.binary)
    matplot.pyplot.show()
